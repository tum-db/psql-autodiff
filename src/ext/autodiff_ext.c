...
PG_FUNCTION_INFO_V1_RECTYPE(autodiff_fast, autodiff_record_type);
...
Datum autodiff_fast_internal(PG_FUNCTION_ARGS, Datum (*derivefunc)(Datum **arg, Datum *derivatives))
{
    MemoryContext oldcontext;
    FuncCallContext *funcctx;
    MemoryContext per_query_ctx;
...
Datum autodiff_fast(PG_FUNCTION_ARGS)
{
    ReturnSetInfo *rsinfo = (ReturnSetInfo *)fcinfo->resultinfo;
    LambdaExpr *lambda = PG_GETARG_LAMBDA(1);
        ...
        lambda->rettype != INT4OID && lambda->rettype != INT8OID)
    {
        ereport(ERROR,
                (errcode(ERRCODE_INTERNAL_ERROR),
                 errmsg("lambda function must return a float8 value. You might need to add type casts.")));
    }
    ...
    return autodiff_fast_internal(fcinfo, compiled_func);
}